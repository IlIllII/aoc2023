input_string = """six1mpffbnbnnlxthree
4eight3one92
9nine2xnhvjtjlzj48
...
answer_1 = sum(resulting_numbers)


def digit_locations(line: str):
    digit_locations = dict()
    for i, char in enumerate(line):
        if char.isdigit():
            digit_locations[i] = int(char)
    for digit in str_digits:
        loc = line.find(digit)
        if loc != -1:
            digit_locations[loc] = digit_to_int(digit)
        loc = line.rfind(digit)
        if loc != -1:
            digit_locations[loc] = digit_to_int(digit)
    return digit_locations


resulting_numbers.clear()
for line in input_string.split("\n"):
    locs = digit_locations(line)
    first, last = min(locs.keys()), max(locs.keys())
    resulting_numbers.append(int(str(locs[first]) + str(locs[last])))

answer_2 = sum(resulting_numbers)
